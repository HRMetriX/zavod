...
ВАЖНО: НИ В КОЕМ СЛУЧАЕ не превышай 1000 символов (включая пробелы и знаки препинания) в основном тексте поста — до строки "PROMPT FOR IMAGE:". Это жёсткое техническое требование Telegram. Если текст превысит 1000 символов — пост будет отклонён, и твоё сообщение не дойдёт до читателей. Поэтому уложись точно в 1000, лучше даже оставить пару десятков символов запаса. В противном случае ты будешь оштрафован
...
Помни: основной текст (до "PROMPT FOR IMAGE:") обязан быть ≤1000 символов. Перед отправкой ответа мысленно сосчитай — если больше, сократи бытовые детали, но сохрани суть и стиль.
...
def generate_image_with_hf(prompt):
    ...
    Учитывает историю предыдущих промптов через negative_prompt для избежания повторов.
    """
    hf_token = os.environ.get("HF_TOKEN")
    if not hf_token:
    ...
    # 1. Подготовить стилевые параметры
    style_part = "Photorealistic, highly detailed, 8k high definition"

    # 2. Сформировать общий промпт: [Описание сцены (от LLM)] + [Стиль]
    # Убираем лишние точки/запятые, которые могут остаться от LLM
    ...
    try:
        ...
        print(f"   -> Full prompt: {full_prompt[:5000]}...") # <-- Лог для отладки (первые 5000 символов, или меньше)
        ...
        # 4. Сохранить ТЕКУЩИЙ чистый промпт (не full_prompt, а исходный от LLM) в историю
